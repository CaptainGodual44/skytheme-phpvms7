...
@php
  ...
  $d_special = check_module('DisposableSpecial');
@endphp

@section('content')
  <div class="row">
    <div class="col-md-8 col-sm-12">
      <div style="font-size: 2.6rem; line-height: 2.6rem;">
        {{ $event->name }}
      </div>
    </div>
          ...
          <div class="btn-group" role="group">
            <div class="dropdown-center">
              ...
              <ul class="dropdown-menu">
                ...
                @endif
                @if($ch_free_flight)
                  <li><a href="{{route('chfreeflight.create', ['route_code' => $event->route_code, 'flight_number' => Auth::user()->pilot_id])}}" class="dropdown-item">Create Free Flight</a></li>
                @endif
              </ul>
            </div>
          </div>
    ...
    <div class="col-4">
      ...
      <div class="card">
        <div class="card-body text-center">
          <div style="font-size: 48px;">{{ $event->users->count() }}</div>
          <div class="description">Users Signed Up</div>
          <hr/>
          @foreach($event->users as $user)
            <span class="badge bg-primary">{{$user->ident}} | {{$user->name_private}}</span>
          @endforeach
        </div>
      </div>
      <div class="card mt-4">
