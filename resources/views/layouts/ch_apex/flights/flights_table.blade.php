      ...
      <div class="card">
        <div class="position-relative hover-action">
          ...
          <div class="hover-container position-absolute w-100 bg-secondary bg-opacity-75 h-100 top-0 d-flex">
            <div class="m-auto">
              <button type="button" class="btn btn-lg btn-primary" title="More Info" data-bs-toggle="modal" data-bs-target="#details_{{$flight->id}}"><i class="bi bi-info-circle"></i></button>
              @if ($acars_plugin)
                @if (isset($saved[$flight->id]))
                  <a href="vmsacars:bid/{{ $saved[$flight->id] }}" title="Load in vmsACARS" class="btn btn-lg btn-primary"><i class="bi bi-download-filled"></i> vmsACARS</a>
                @else
                  <a href="vmsacars:flight/{{ $flight->id }}" title="Load in vmsACARS" class="btn btn-lg btn-primary"><i class="bi bi-download-filled"></i> vmsACARS</a>
                @endif
              @endif
              <!-- Simbrief enabled -->
              @if ($simbrief !== false)
                ...
                @if ($flight->simbrief && $flight->simbrief->user_id === $user->id)
                  <a href="{{ route('frontend.simbrief.briefing', $flight->simbrief->id) }}" title="View SimBrief OFP"
                ...
                @else
                  ...
                  @if ($simbrief_bids === false || ($simbrief_bids === true && isset($saved[$flight->id])))
                    ...
                    <a href="{{ route('frontend.simbrief.generate') }}?flight_id={{ $flight->id }}@if($aircraft_id)&aircraft_id={{ $aircraft_id }} @endif"
                       class="btn btn-lg btn-primary" title="Create SimBrief OFP">
                      <i class="bi bi-plus-lg"></i>&nbsp;OFP
                    </a>
                  @endif
                @endif
              @endif
              <a href="{{ route('frontend.pireps.create') }}?flight_id={{ $flight->id }}" title="{{__('pireps.newflightreport')}}"
